...
def TTC(agents):
    ...
    while len(agents_remaining) > 0:
        ...
        while len(unknown) > 0:
            ...
            logging.debug("isCycle: " + str([[(agent.id, agent.item) for agent in cycle] for cycle in isCycle]))
            logging.debug("isNotCycle " + str([(agent.id, agent.item) for agent in isNotCycle]))
            ...
            if next_agent.id in [agent.id for agent in cycle_track]:
                ...
                start_index = [agent.id for agent in cycle_track].index(next_agent.id)

                logging.debug("next_agent in cycle_track, index "+str(start_index))

                logging.debug("current unknown: " + str([(agent.id, agent.item) for agent in unknown]))
                ...
                if len(unknown) > 0:
                    cycle_track = [unknown[0]]
                    logging.debug("reset cycle_track, now: "+str([(agent.id, agent.item) for agent in cycle_track]))
                ...
                logging.debug("current cycle_track: " + str([(agent.id, agent.item) for agent in cycle_track]))
            else:
                ...
                logging.debug("POST REMOVAL")
                logging.debug("agent " + str(agent.id) + " appended to isNotCycle and removed from unknown")
                logging.debug("current isNotCycle: " + str([(agent.id, agent.item) for agent in isNotCycle]))
                logging.debug("current isCycle: " + str([[(agent.id, agent.item) for agent in cycle] for cycle in isCycle]))
                logging.debug("current agents_remaining: " + str([(agent.id, agent.item) for agent in agents_remaining]))
                logging.debug("current unknown: " + str([(agent.id, agent.item) for agent in unknown]))
        ...
        for cycle in isCycle:
            ...
            for i in range(len(cycle)):
                ...
                logging.debug("agent " + str(i) + " is assigned item " + str(agents[i].item))
