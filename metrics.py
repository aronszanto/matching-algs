...
def IsParetoOptimal(agents, items):
        ...
        # assignment is something like (0,2,1) -> agent 0 get item 0; agent 1
        # gets item 2; agent 2 gets item 1

        # check if this assignment Pareto dominates the current assignment.
        # Yes, if everybody is at least better off and it is not the same
        # assignment (so somebody is strictly better off)
    for assignment in permutations(items):
                # if the same assignment, continue (this is not gonna get you a
                # Pareto dominating assignment)
        if list(assignment) == items:
            continue

        for i in range(len(items)):
            # Is curr_agent better off? If NO, assignment doesn't Pareto
            # dominate, continue
            curr_agent = agents[i]
    ...
    # if you survive the big permutation loop, YES the current is Pareto
    # Optimal
    return True
